...
public class CustomerInvoiceController : ApiControllerBase
{
    ...
    public CustomerInvoiceController(ICustomerInvoiceService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCustomerInvoiceDto dto)
    {
    ...
    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateCustomerInvoiceDto dto) =>
    ...
    [Authorize(Policy = "finance.edit")]
    [HttpPost("{id}/receive-payment")]
    public async Task<IActionResult> ReceivePayment(int id, [FromBody] ReceiveCustomerInvoicePaymentDto dto) =>
        Ok(await _service.ReceivePaymentAsync(OrgId, id, dto));
