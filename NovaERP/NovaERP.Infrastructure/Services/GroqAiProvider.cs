...
/// <summary>Calls Groq's OpenAI-Chat-Completions-compatible endpoint
/// (https://console.groq.com/docs/api-reference#chat-create). Always offers get_widget_data;
/// offers propose_create_service_ticket only when AiToolContext.CanCreateServiceTickets is true,
/// so an unauthorized caller's model literally never has the option to call it. Reads the API
/// key/model from the per-organization AiAssistantSettings row at call time, same send-time DB
/// read as AnthropicAiProvider/HttpSmsSender.</summary>
public class GroqAiProvider : IAiProvider
{
    ...
    public GroqAiProvider(HttpClient httpClient, NovaErpDbContext ctx)
    {
    ...
    public async Task<AiCompletionResult> CompleteAsync(int orgId, List<AiChatTurn> history, AiToolContext toolContext, CancellationToken ct = default)
    {
        ...
        var apiKey = settings?.GroqApiKey ?? string.Empty;
        var model = settings?.GroqModel;
        if (string.IsNullOrWhiteSpace(model)) model = "llama-3.3-70b-versatile";

        var messages = new List<object> { new { role = "system", content = SystemPrompt } };
        messages.AddRange(history.Select(ToOpenAiMessage));
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            {
                401 => "The Groq API key is invalid or has been rejected — ask an administrator to check the key in AI Assistant Settings.",
                429 => "The Groq API rate limit was hit — please try again in a moment.",
        ...
        if (message.TryGetProperty("tool_calls", out var toolCalls) && toolCalls.ValueKind == JsonValueKind.Array && toolCalls.GetArrayLength() > 0)
        {
            ...
            toolCallId = call.GetProperty("id").GetString();
            toolCallName = call.GetProperty("function").GetProperty("name").GetString();
            toolCallArgumentsJson = call.GetProperty("function").GetProperty("arguments").GetString();
        }
        else if (message.TryGetProperty("content", out var contentEl) && contentEl.ValueKind == JsonValueKind.String)
        {
            finalText = contentEl.GetString();
        }
    ...
    private static object ToOpenAiMessage(AiChatTurn turn)
    {
